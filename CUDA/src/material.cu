...
__host__ Metal::Metal(color alb, float f){
    this->albedo = alb;
    this->fuzz = f;
}

__host__ Glass::Glass(color alb, float ir){
    this->albedo = alb;
    this->ir = ir;
}
...
__device__ bool Glass::scatter(const ray& r_in, vec3 normal, point3 p, color& attenuation, ray& scattered, curandState *curand_States){
    ...
    if (cannot_refract || reflectance(cos_theta, refraction_ratio) > curand_uniform_float(curand_States)){
...
__device__ float Glass::reflectance(float cosine, float ref_idx) {
